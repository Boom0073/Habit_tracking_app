...
class ThemeSelectionList extends StatefulWidget {
  const ThemeSelectionList({
    Key? key,
    required this.currentThemeSetting,
    required this.availableWidth,
    this.onColorIndexSelected,
    this.onVariantIndexSelected,
  }) : super(key: key);
  final AppThemeSettings currentThemeSetting;
...
class _ThemeSelectionListState extends State<ThemeSelectionList> {
  ...
  double get scrollOffset {
    ...
    final selectedIndex = widget.currentThemeSetting.colorIndex;
    ...
    return ListView(
      ...
      children: [
        for (final color in allColors)
          ThemeVariantPicker(
            color: color,
            isSelected: widget.currentThemeSetting.colorIndex ==
                allColors.indexOf(color),
            selectedVariantIndex: widget.currentThemeSetting.variantIndex,
            onColorSelected: (color) {
              final newIndex = allColors.indexOf(color);
              final previousIndex = widget.currentThemeSetting.colorIndex;
              if (previousIndex != newIndex) {
                widget.onColorIndexSelected?.call(allColors.indexOf(color));
              } else {
                final newVariantIndex =
                    (widget.currentThemeSetting.variantIndex + 1) % 3;
