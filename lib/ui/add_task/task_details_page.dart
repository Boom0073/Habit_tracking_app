...
//*Note: extending ConsumerStatefulWidget so that we can access the WidgetRef
...
class _ConfirmTaskContentsState extends ConsumerState<ConfirmTaskContents> {
  ...
  Future<void> _saveTask() async {
    ...
    if (textFieldState != null) {
      ...
      final task = Task(
          id: widget.task.id, name: textFieldState.text, iconName: _iconName);
      try {
        ...
        //*Pop back to Homepage, using 'rootNavigator: true' to ensure we dismiss the entire
        //navigation stack.
        if (!mounted) return;
  ...
  Future<void> _deleteTask() async {
    ...
    if (didConfirm == true) {
      try {
        ...
        //* Pop back to HomePage, using 'rootNavigator: true' to ensure we dismiss the entire navigation
        //stack.
        if (!mounted) return;
        Navigator.of(context, rootNavigator: true).pop();
      } catch (e) {
        ...
        SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const TextFieldHeader(text: 'TITLE'),
              CustomTextField(
                key: _textFieldKey,
